...
function App() {
    ...
    useEffect(() => {
        if (window.localStorage.getItem("customApiKey")) {
            dispatch({
                payload: {
                    name: window.localStorage.getItem("username"), apiKey: window.localStorage.getItem("customApiKey"),
        ...
        fetch(GetAPIUrl(state.userProfile.apiKey))
            .then((response) => response.json().then((resData) => {
                dispatch({
                    payload: ConvertAPIDataToList(resData), type: "UPDATE_ASTEROIDS_LIST",
                });
            }))
